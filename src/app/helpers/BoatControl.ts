...
import { KeyboardController } from '../../tsgl/input/GameInput';
...
const inputState = new KeyboardController();
...
export default {
  ...
  update(boat: b2Body): void {
    ...
    if (this.inputEnabled) {
      if (inputState.keys.d_pad_left) {
        boat.ApplyTorque(directionS);
      } else if (inputState.keys.d_pad_right) {
        boat.ApplyTorque(-directionS);
      }

      if (inputState.keys.d_pad_up || inputState.keys.d_pad_down || inputState.keys.button_3 || inputState.keys.button_1) {
        if (inputState.keys.d_pad_up || inputState.keys.button_3) {
      ...
      throttleAngleV += tAcc * (inputState.keys.d_pad_up ? 1 : -1);
    } else this.engineState = 0;
