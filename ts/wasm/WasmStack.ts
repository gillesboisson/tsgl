...
export type WasmBufferStateUpdateFunc = (ptr: number, index: number, length: number) => void;
...
export class WasmStack extends WasmClass {
  ...
  @structAttr({ length: 5, type: Uint32Array })
  ...
  constructor(bufferStride: number, bufferStep = DEFAULT_BUFFER_LENGTH_STEP, module?: EmscriptenModuleExtended) {
    super(module);
  ...
  getBuffer() {
    const metas = this.metas;
    if (this.__jsBufferLength !== metas[1] || this.__jsBufferPtr !== metas[3] || this._buffer === undefined)
  ...
  protected syncBuffer(metas: Uint32Array) {
    ...
    this._buffer = new Uint8Array(this._module.HEAP8.buffer, metas[3], metas[1] * metas[4]);
  }

  resize(newLength: number, autoShrink = this.autoShrinkBuffer): number {
    if (newLength < 0) newLength = 0;
    const metas = this.metas;
    const newBufferLength = newLength > 0 ? Math.ceil(newLength / metas[2]) * metas[2] : metas[2];

    if (newBufferLength > metas[1] || (autoShrink && newBufferLength !== metas[1])) {
      metas[3] = (<EmscriptenModuleExtended>this._module)._realloc(metas[3], newBufferLength * metas[4]);
