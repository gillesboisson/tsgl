  ...
  for (prop of prototype.__anPropsList)
    if (prop.defined === false) {
      ...
      if (prop.useAccessor) {
        ...
        let get, set;
        if (prop.wasm !== undefined && prop.wasm.isPtr === true) {
        ...
        if (prop.wasm !== undefined && prop.wasm.wasmType !== undefined) {
          ...
          set = function(val: any) {
            if (val && val !== this[propName]) {
              //const newVal = new prop.wasmType(this._module,this[propNamePtr]);
